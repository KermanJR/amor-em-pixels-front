...
const Create = () => {
  ...
  const handleNext = () => {
    ...
    const values = form.getValues();
    if (activeStep === 0) {
      if (!selectedPlan) return toast({ title: 'Erro', description: 'Selecione um plano.', variant: 'destructive' });
    } else if (activeStep === 1) {
      if (!values.coupleName || !values.relationshipStartDate || errors.coupleName || errors.relationshipStartDate) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {Object.entries(PLANS).map(([plan, { photos, videos, musics, price, durationMonths }]) => (
              <motion.div
                key={plan}
                className={cn(
                  'p-6 rounded-2xl shadow-lg border cursor-pointer transition-all duration-300',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-pink-50 to-purple-50">
      <Navbar />
      <div className="container mx-auto py-16 px-4 lg:px-8 grid lg:grid-cols-2 gap-12">
        {/* Formulário com Steps */}
        <motion.div
          className="bg-white p-8 rounded-3xl shadow-2xl border border-pink-100/50 max-w-lg mx-auto"
          initial={{ opacity: 0, y: 50 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
        >
          <h1 className="text-3xl font-serif text-transparent bg-clip-text bg-gradient-to-r from-pink-600 to-purple-800 mb-8 text-center">
              ...
              <div className="flex justify-between mb-8">
                {steps.map((step, index) => (
                  ...
                  >
                    <div
                      className={cn(
                        'w-10 h-10 mx-auto rounded-full flex items-center justify-center font-bold text-lg mb-2',
                        index < activeStep ? 'bg-pink-600 text-white' : index === activeStep ? 'bg-gradient-to-r from-pink-600 to-purple-800 text-white' : 'bg-gray-200'
                      )}
                    >
                      {index + 1}
                    </div>
        ...
        {/* Pré-visualização */}
        <motion.div
          className="bg-white p-6 rounded-3xl shadow-2xl border border-pink-100/50 max-w-lg mx-auto lg:sticky lg:top-16"
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.8, delay: 0.2 }}
        >
          <h2 className="text-2xl font-serif text-transparent bg-clip-text bg-gradient-to-r from-pink-600 to-purple-800 mb-6 text-center">
            Pré-visualização
          </h2>
          <div className="relative h-[600px] overflow-y-auto rounded-xl border border-gray-200 shadow-inner">
            {previewData && previewData.formData.coupleName ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-500 bg-gray-50">
                <Sparkles className="h-12 w-12 animate-twinkle mb-4" />
                <p className="text-lg text-center">Comece preenchendo os campos para ver seu Card Digital ganhar vida!</p>
              </div>
            )}
          </div>
        </motion.div>
